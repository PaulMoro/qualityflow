...
export default function ConfigurationPanel() {
  ...
  return (
    <Tabs defaultValue="project-types" className="space-y-4">
      <TabsList className="grid w-full grid-cols-3 bg-[var(--bg-tertiary)] border-[var(--border-primary)]">
        <TabsTrigger value="project-types" className="data-[state=active]:bg-[#FF1B7E] data-[state=active]:text-white text-[var(--text-secondary)]">
          <Briefcase className="h-4 w-4 mr-2" />
          Tipos de Proyecto
        </TabsTrigger>
        <TabsTrigger value="fee-types" className="data-[state=active]:bg-[#FF1B7E] data-[state=active]:text-white text-[var(--text-secondary)]">
          <DollarSign className="h-4 w-4 mr-2" />
          Tipos de Fee
        </TabsTrigger>
        <TabsTrigger value="clients" className="data-[state=active]:bg-[#FF1B7E] data-[state=active]:text-white text-[var(--text-secondary)]">
      ...
      <TabsContent value="project-types" className="space-y-4">
        <Card className="bg-[var(--bg-secondary)] border-[var(--border-primary)]">
          <CardHeader>
            <CardTitle className="text-base text-[var(--text-primary)]">Agregar Tipo de Proyecto</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <div className="space-y-2">
          {projectTypes.map((type) => (
            <Card key={type.id} className="bg-[var(--bg-secondary)] border-[var(--border-primary)]">
              <CardContent className="py-3 flex items-center justify-between">
                <div>
                  <p className="font-medium text-[var(--text-primary)]">{type.name}</p>
                  <p className="text-sm text-[var(--text-secondary)]">{type.description || type.key}</p>
                </div>
                <Button
                  size="icon"
                  variant="ghost"
      ...
      <TabsContent value="fee-types" className="space-y-4">
        <Card className="bg-[var(--bg-secondary)] border-[var(--border-primary)]">
          <CardHeader>
            <CardTitle className="text-base text-[var(--text-primary)]">Agregar Tipo de Fee</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <TabsContent value="clients" className="space-y-4">
        <Card className="bg-[var(--bg-secondary)] border-[var(--border-primary)]">
          <CardHeader>
            <CardTitle className="text-base text-[var(--text-primary)]">Agregar Cliente/Sociedad</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <div className="space-y-2">
          {clients.map((client) => (
            <Card key={client.id} className="bg-[var(--bg-secondary)] border-[var(--border-primary)]">
              <CardContent className="py-3">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-medium text-[var(--text-primary)]">{client.name}</p>
                    {client.contact_email && (
                      <p className="text-sm text-[var(--text-secondary)]">{client.contact_email}</p>
                    )}
                    {client.contact_phone && (
                      <p className="text-sm text-[var(--text-secondary)]">{client.contact_phone}</p>
                    )}
                  </div>
                  <Button
                    size="icon"
